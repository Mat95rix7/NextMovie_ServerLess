...
import { useState } from 'react';
import { Trash2, Edit, Save, X, ToggleLeft, ToggleRight } from 'lucide-react';
import ConfirmationModal from './ConfirmationModal';
import { deleteUserAccount, updateProfile } from '../../hooks/userProfile';
import PropTypes from 'prop-types';
      ...
      <tr key={user.id}>
        ...
        <td className="px-6 py-4 whitespace-nowrap text-center text-sm text-gray-500">
          ...
          >
            {isActive ? (
              <>
                <ToggleRight className="h-5 w-5 mr-1" /> Activé
              </>
            ) : (
              <>
                <ToggleLeft className="h-5 w-5 mr-1" /> Désactivé
              </>
            )}
          </button>
        </td>
